...
public class SimplePlanner : MonoBehaviour
{
    ...
    public void PlanTrajectory(
        ...
        if(debugVisual)
        {
            ...
            VisualizeStartAndGoal(
                startPosition, startRotation, goalPosition, goalRotation
            );
        }
        ...
        {
            Debug.LogWarning(
                "Imminent Collision Detected, No Direct Path Found"
            );
            return;
        }
        ...
        List<Vector3> positions = InterpolatePositions(
            startPosition, goalPosition
        );
        List<Quaternion> rotations = InterpolateRotations(
            startRotation, goalRotation
        );

        // Generate an array of waypoints between Start and Goal
        ...
        if(debugVisual)
        {
            ...
            {
                VisualizeWaypoint(
                    waypointsPositions[i], waypointsRotations[i]
                );
            }
        }
        ...
        float[] jointAngle = armController.GetJointAngles();
        ...
        for (int i = 0; i < waypointsPositions.Count; i++)
        {
            ...
            jointAngle = inverseKinematics.SolveIK(
                jointAngle, waypointsPositions[i], waypointsRotations[i]
            );
            jointAngles.Add(jointAngle);
        ...
        {
            Debug.Log(
                "Goal Arm Configuration Not Achievable, No Trajectory Sent"
            );
            return;
        }

        // Call SetJointTrajectory method
        // Goal Arm Configuration Achievable, sending trajectory
        armController.SetJointTrajectory(
            timeStepsArray, 
            jointAnglesArray, 
            new float[][] { }, 
            new float[][] { }
        ...
        StartCoroutine(
            CheckGoalReached(
                timeStepsArray[timeStepsArray.Length - 1], goalPosition
            )
        );
    }
    ...
    private List<Quaternion> InterpolateRotations(
        Quaternion start, Quaternion goal
    ) {
    ...
    private void VisualizeStartAndGoal(
        Vector3 startPosition, 
        Quaternion startRotation, 
        Vector3 goalPosition, 
        Quaternion goalRotation
        ...
        Instantiate(
            ...
            startPosition, 
            startRotation, 
            startGameObject.transform.parent
        );
        Instantiate(
            goalGameObject, 
            goalPosition, 
            goalRotation, 
            goalGameObject.transform.parent
        );
    }
    ...
    IEnumerator CheckGoalReached(float timer, Vector3 goal)
    {
        ...
        while(timeElapsed < timer + 5f)
        {
            ...
            if (distance < 0.01f)
            {
                ...
                // Signal that we are done, ready to move on
                goalReached = true;
        ...
        // failure. 
        // We didn't get to the goal in the expected time
        goalReached = false;
        motionInProgress = false;
    }

    public bool CheckGoalConfiguration(
        float[] jointAngles, Vector3 goalPosition, Quaternion goalRotation
    ) {
        ...
        float positionError = Vector3.Distance(eePosition, goalPosition);
        float rotationError = Quaternion.Angle(eeRotation, goalRotation);

        // Check convergence
        return positionError < 1e-3 && rotationError * Mathf.Deg2Rad < 2e-2;
    }
}
