...
{
    // Articulation Body
    [SerializeField] private ArticulationBody[] jointRoots;
    ...
    // Sound parameters
    [SerializeField] private float lowerFilter = 0.5f;
    ...
    void Update()
    {
        for (int i=0; i<allJointsList.Length; ++i)
        {
            ...
            {
                jointAudio.volume = Mathf.Lerp(
                    jointAudio.volume, 0f, Time.deltaTime * fadeSpeed
                );
            }
            else if (speed > lowerFilter && jointAudio.volume != volume)
            {
                jointAudio.volume = Mathf.Lerp(
                    jointAudio.volume, volume, Time.deltaTime * inSpeed
                );
            }
    ...
    private void AddToJointList(ArticulationBody[] newList)
    {
        ...
        {
            ArticulationBody[] resultList = new ArticulationBody[
                allJointsList.Length + newList.Length
            ];
